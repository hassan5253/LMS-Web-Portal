...
using System.Data;
using READBModel;
public partial class Staff_frm_AssignmentDetails_Staff : System.Web.UI.Page
{
    ...
    protected void lnkBtn_Edit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Response.Redirect("frm_ManageUsersEdit_Staff.aspx?ID=" + ID);
    ...
    protected void lnkBtn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            using (READBEntities cntx = new READBEntities())
            {
                ...
                {
                    obj_Search = (from q in cntx.tblAssignmentDetails where q.a_id == A_ID && q.student_roll_no.Contains(txt_Search.Text) select q).ToList();
                }
    ...
    protected void lnkBtn_Marks_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (READBEntities cntx = new READBEntities())
            {
                GridViewRow clickedRow = ((LinkButton)sender).NamingContainer as GridViewRow;
                int ID = Convert.ToInt32(gv_AssDetails.DataKeys[clickedRow.RowIndex].Value);
                Response.Redirect("frm_AssignmentMarks_Staff.aspx?ad_id=" + ID+"&a_id="+hf_AID.Value);
    ...
    protected void lnkBtn_DeleteAll_Click(object sender, EventArgs e)
    {
        ...
        using (READBEntities cntx = new READBEntities())
        {
            ...
            {
                //GridViewRow clickedRow = ((LinkButton)sender).NamingContainer as GridViewRow;
                int ID = Convert.ToInt32(gv_AssDetails.DataKeys[xRow.RowIndex].Value);
                CheckBox cb = (CheckBox)xRow.FindControl("cb_Middle") as CheckBox;
    ...
    private void Fill_Grid()
    {
        try
        {
            using (READBEntities cntx = new READBEntities())
            {
                ...
                long User_ID;
                DataTable Dt = GlobalClass.CreateDataTable("ad_id,name,student_roll_no,ad_uploaded_date,ad_marks");
                var obj_Assignments = (from q in cntx.tblAssignmentDetails where q.a_id==A_ID select q).ToList();
