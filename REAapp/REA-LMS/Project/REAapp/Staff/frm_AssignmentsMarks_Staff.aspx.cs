...
using READBModel;
using System.Data;
public partial class Staff_frm_AssignmentsMarks_Staff : System.Web.UI.Page
{
        ...
        try
        {
            using (READBEntities cntx = new READBEntities())
            {
                ...
                obj_Search = (from q in cntx.tblAssignments
                              ...
                              where a.student_roll_no.Contains(txt_Search.Text)
                              select q).ToList();
    ...
    private void Fill_Grid()
    {
        try
        {
            using (READBEntities cntx = new READBEntities())
            {
                ...
                DataTable Dt = GlobalClass.CreateDataTable("a_id,a_name,a_assigned_by,a_assigned_date,a_submit_date");
                var obj_Assignments = (from q in cntx.tblAssignments where q.a_assigned_by==obj_User.name  select q).ToList();
